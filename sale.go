...
func (c *Client) RefundSale(saleID string, a *RefundRequest) (*Refund, error) {

	refund := &Refund{}

	req, err := c.NewRequest("POST", fmt.Sprintf("%s%s", c.APIBase, "/v2/payments/sale/"+saleID+"/refund"), a)
	if err != nil {
		return refund, err
	}
